# ADR-0001: Use AI-Assisted Tooling for ADR Creation

**Status:** Accepted - Phase 1 (Skill) - 2025-01-16
...
This project follows an AI-native approach (see README.md "Why AI-Native?"). The same principle applies to ADR tooling:
- AI agents should assist with ADR creation, not just consume them
- Tooling should reduce token waste at each layer (Responsibility Matrix principle)
...
- `scripts/update-adr-toc.py`: Python 3.12+ script for automatic TOC generation
...
- Demonstrates the Responsibility Matrix principle (Skills for context, Scripts for validation)
...
2. **Python Dependency**: Contributors need Python 3.12+ for development
...
3. **Reusability**: ADR Skill becomes part of the plugin, usable by all three user groups (app developers, adapter developers, core maintainers)
4. **Meta-Bootstrapping**: This ADR documents the tooling used to create subsequent ADRs

## References

- [ADR-0000: Use ADRs to Document Architecturally Significant Decisions](adr-0000-use-adrs-to-document-architecturally-significant-decisions.md)
...
- [ADR TOC Update Script](../../scripts/update-adr-toc.py)
